...
namespace calc {
    ...
    namespace detail {
        ...
        struct matrix_mul_impl<float> {
            static void mul(const __m128& r, const __m128& c, float* out)
            {
                __m128 tmp;
    ...
    struct matrix_mul<float, 2, 2, 2> {
        static void mul(const float* dat1, const float* dat2, float* out)
        {
            ...
            __m128 r0 = _mm_loadu_ps(dat1);
            __m128 r1 = _mm_loadu_ps(dat1 + 2);

            detail::matrix_mul_impl<float>::mul(r0, c0, out);
            detail::matrix_mul_impl<float>::mul(r0, c1, out + 1);

            detail::matrix_mul_impl<float>::mul(r1, c0, out + 2);
            detail::matrix_mul_impl<float>::mul(r1, c1, out + 3);
        }
    };
} // namespace calc
